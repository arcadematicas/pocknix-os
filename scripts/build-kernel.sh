...
stage_dtbs() {
  ...
  # Also stage the Odin 3 (AYN CQ8725S) DTB, which is generated by the ROCKNIX
  # patch stack (0046/0047) rather than shipped as a standalone .dts in
  # kernel/${SOC}/dts/. Without it the ABL cannot match the Odin 3 board ID and
  # refuses to boot ("cannot boot this system").
  for extra in cq8725s-ayn-odin3; do
    src="$(find "${KSRC}/arch/arm64/boot/dts" -name "${extra}.dtb" -print -quit)"
    if [ -n "${src}" ]; then
      cp "${src}" "${KBUILD}/out/dtbs/"
      n=$((n+1))
      log "  staged extra dtb: ${extra}.dtb"
    else
      warn "extra dtb ${extra}.dtb not built — Odin 3 boot may fail"
    fi
  done
