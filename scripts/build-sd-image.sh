...
PARTUUID=${SD_ROOT_PARTUUID}  /home              btrfs  rw,noatime,compress=zstd:1,commit=5,subvol=@home          0 0
PARTUUID=${SD_ROOT_PARTUUID}  /.snapshots        btrfs  rw,noatime,compress=zstd:1,commit=5,subvol=@snapshots     0 0
  PARTUUID=${SD_ROOT_PARTUUID}  /var/cache/pacman  btrfs  rw,noatime,nodatacow,commit=5,subvol=@pacman-cache  0 0
  PARTUUID=${SD_ROOT_PARTUUID}  /var/log           btrfs  rw,noatime,nodatacow,commit=5,subvol=@var-log       0 0
...
EOF
  ...
  # The on-device launcher has no network fallback: a rootfs without the baked client would ship a
  # Steam session that fails on first launch. The native client is pre-extracted into /home/deck by
  # build-image.sh's bootstrap_steam_seed (done there so `du -sm ROOTFS_DIR` above sizes the
  # partition to fit the ~1.3 GB tree); the chown above (root -> deck) gives deck ownership of it.
  if [ "${SOC}" = "sm8750" ]; then
    # SM8750: Steam support is still maturing — warn rather than block, so a display/wake test
    # image can still be produced.
    [ -x "${root}/home/deck/.local/share/Steam/steamrtarm64/steam" ] \
      || warn "Steam client not pre-extracted — gaming packages may not be built for SM8750 (ALARM defaults only)"
  else
    [ -x "${root}/home/deck/.local/share/Steam/steamrtarm64/steam" ] \
